    ...
    <div className="space-y-12 animate-in slide-in-from-right duration-500 pb-20">
      ...
      <div className="glass p-6 rounded-xl border border-zinc-800/50">
        ...
        <button
          onClick={handleCalc}
          disabled={loading}
          className="w-full py-4 bg-blue-600 hover:bg-blue-700 disabled:bg-zinc-800 text-white font-bold rounded-lg transition-all shadow-lg flex items-center justify-center gap-2 uppercase font-oswald tracking-widest"
        >
      ...
      {macros && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in zoom-in-95 duration-300">
          <div className="lg:col-span-1 glass p-8 rounded-xl flex flex-col items-center justify-center text-center bg-blue-600/5">
          ...
          <div className="lg:col-span-2 glass p-8 rounded-xl border border-zinc-800/50">
             <h3 className="text-blue-200/40 text-sm font-bold tracking-widest mb-8 uppercase">ANABOLIC MACRO SPLIT</h3>
             <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
               <div className="text-center group">
                 ...
                 <span className="block text-4xl font-oswald font-bold text-white group-hover:text-blue-500 transition-colors">{macros.protein}G</span>
                 <span className="text-blue-300/40 text-[10px] font-bold tracking-widest uppercase">PROTEIN</span>
               </div>
               <div className="text-center group">
                 <div className="w-full bg-zinc-900 h-2 rounded-full mb-3 overflow-hidden">
                   <div className="bg-cyan-600 h-full w-[100%] group-hover:bg-cyan-400 transition-colors"></div>
                 </div>
                 <span className="block text-4xl font-oswald font-bold text-white group-hover:text-cyan-500 transition-colors">{macros.carbs}G</span>
               ...
               <div className="text-center group">
                 ...
                 <span className="block text-4xl font-oswald font-bold text-white group-hover:text-blue-300 transition-colors">{macros.fats}G</span>
                 <span className="text-blue-300/40 text-[10px] font-bold tracking-widest uppercase">FATS</span>
               </div>
             </div>
             {resultText && (
               <div className="mt-8 p-4 bg-zinc-900/30 rounded-lg border border-zinc-800/50 text-sm text-blue-100/40 leading-relaxed italic border-l-2 border-l-blue-600">
                 {resultText}
               </div>
             )}
          </div>
      ...
      <div className="glass p-6 rounded-xl border border-zinc-800/50">
        ...
        <p className="text-blue-200/40 text-sm mb-6">Type any meal or food item to get a bodybuilding-specific nutritional breakdown.</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <button
            ...
            className="px-8 py-4 bg-cyan-600 hover:bg-cyan-700 disabled:bg-zinc-800 text-white font-bold rounded-lg transition-all flex items-center justify-center gap-2 uppercase font-oswald tracking-widest"
          >
            {foodLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="md:col-span-1 glass bg-zinc-900/40 p-6 rounded-xl text-center border border-zinc-800 relative">
                <div className={`absolute top-2 right-2 w-10 h-10 rounded-full flex items-center justify-center font-oswald text-xl font-bold border-2 ${
                  foodResult.grade.startsWith('A') ? 'border-cyan-500 text-cyan-500' : 
                  foodResult.grade.startsWith('B') ? 'border-blue-500 text-blue-500' : 
                  'border-blue-900/50 text-blue-900'
                }`}>
                  {foodResult.grade}
                </div>
                <h4 className="text-blue-300/40 text-[10px] font-bold tracking-widest mb-1 uppercase">ANALYZE RESULT</h4>
                <p className="text-xl font-oswald font-bold text-white mb-4 line-clamp-1">{foodResult.foodName}</p>
                <p className="text-blue-200/40 text-xs mb-4">Serving: {foodResult.servingSize}</p>
                <div className="text-3xl font-oswald font-bold text-white">{foodResult.calories}</div>
                <p className="text-blue-300/40 text-[10px] font-bold tracking-widest">KCAL</p>
              </div>

              <div className="md:col-span-3 space-y-6">
                <div className="grid grid-cols-3 gap-4">
                  <div className="p-4 bg-zinc-900/60 border border-zinc-800 rounded-xl text-center">
                    <span className="text-[10px] font-bold text-blue-500 block mb-1 uppercase tracking-widest">PROTEIN</span>
                    <span className="text-2xl font-oswald font-bold text-white">{foodResult.protein}g</span>
                  </div>
                  <div className="p-4 bg-zinc-900/60 border border-zinc-800 rounded-xl text-center">
                    <span className="text-[10px] font-bold text-cyan-500 block mb-1 uppercase tracking-widest">CARBS</span>
                    <span className="text-2xl font-oswald font-bold text-white">{foodResult.carbs}g</span>
                  </div>
                  <div className="p-4 bg-zinc-900/60 border border-zinc-800 rounded-xl text-center">
                    <span className="text-[10px] font-bold text-slate-500 block mb-1 uppercase tracking-widest">FATS</span>
                    <span className="text-2xl font-oswald font-bold text-white">{foodResult.fats}g</span>
                  </div>
                </div>
                
                <div className="p-5 glass border-l-4 border-l-cyan-600 bg-cyan-900/5">
                   <h5 className="text-[10px] font-bold text-blue-300/40 tracking-widest uppercase mb-2">ABEBE'S VERDICT</h5>
                   <p className="text-blue-100 italic text-sm leading-relaxed">"{foodResult.coachTip}"</p>
                </div>
              </div>
            </div>
          </div>
                ...
                {cat.foods.map((food, fIdx) => (
                  <div key={fIdx} className="p-4 hover:bg-zinc-800/30 transition-colors group">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-[10px] font-bold text-blue-400/50">{food.cal} KCAL</span>
                    </div>
                    <p className="text-[10px] text-blue-300/40 mb-3 uppercase tracking-tighter">{food.size}</p>
                    <div className="grid grid-cols-3 gap-2">
                      <div className="text-[9px] font-bold">
                        <span className="text-blue-500 mr-1">P</span>
                        <span className="text-zinc-300">{food.p}g</span>
                      </div>
                      <div className="text-[9px] font-bold">
                        <span className="text-cyan-500 mr-1">C</span>
                        <span className="text-zinc-300">{food.c}g</span>
                      </div>
                      <div className="text-[9px] font-bold">
                        <span className="text-blue-200/50 mr-1">F</span>
